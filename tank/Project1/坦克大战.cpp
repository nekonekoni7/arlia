...
#define WIDTH 10					//组成的的正方形的边长
#define RANGE 50					//游戏场地与客户去之间间隔
#define SNAKE_COLOR RGB(235,110,220)	  //坦克的颜色
...
void MakeParttank(int, int, int, GRID *);	//为坦克增加或设置一个方块
void Movetank(HWND);
void Moveshot(HWND);
	...
	int iCmdShow)
{
	...
	hwnd = CreateWindow(szAppName,
		TEXT("tank war by xiaolong V1.0.1"),
...
void Movetank(HWND hwnd)
{
	...
	for (int i = 0; i != 4; ++i)
	{
		...
		if (flags[(tank + i)->x][(tank + i)->y] == EMEY)
		{
			FFLAG = 0;
			MessageBox(NULL, TEXT("you have been slain"), szAppName, MB_OK);
			scroe--;
			initializer();
